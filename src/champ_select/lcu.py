...
def register(connector: Connector):
    ...
    async def on_champ_select_update(conn: Connection, event: WebsocketEventResponse):
        try:
            ...
            logger.exception("Exception on UPDATE /lol-champ-select/v1/session")
    ...
    async def on_champ_select_delete(conn: Connection, event: WebsocketEventResponse):
        ...
        try:
            logger.info("DELETE /lol-champ-select/v1/session")
    ...
    async def on_aram_champs_create(conn: Connection, event: WebsocketEventResponse):
        try:
            logger.info(
                "CREATE /lol-lobby-team-builder/champ-select/v1/subset-champion-list"
            )
            push(
            ...
            logger.exception(
                "Exception on CREATE /lol-lobby-team-builder/champ-select/v1/subset-champion-list"
            )
    ...
    async def on_champ_selected(conn: Connection, event: WebsocketEventResponse):
        try:
            logger.info("CREATE /lol-champ-select/v1/current-champion")
            push(
                '<div id="aram-picks" hx-swap-oob="true" style="display: none;"></div>'
            )
        except Exception:
            logger.exception(
                "Exception on CREATE /lol-champ-select/v1/current-champion"
            )
    ...
    async def on_skin_change(conn: Connection, event: WebsocketEventResponse):
        try:
            logger.info("CREATE/UPDATE /lol-champ-select/v1/skin-selector-info")
            push(
            ...
            logger.exception(
                "Exception on CREATE/UPDATE /lol-champ-select/v1/skin-selector-info"
            )
...
def switch_screen():
    try:
        logger.info("Switching to champ select")
        push(open(Path("src/champ_select/index.html"), encoding="utf-8").read())
    except Exception:
        logger.exception("Exception when switching to champ select")
...
async def get_allowed_swaps(conn: Connection) -> list[dict[int, bool]]:
    try:
        ...
        for i, endpoint in enumerate(endpoints):
            ...
            if resp.status != 200:
                logger.warning(
                    f"Responce status code {resp.status} when trying to get {endpoint}"
                )
                continue
