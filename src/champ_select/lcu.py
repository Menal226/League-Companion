...
def create_my_team_player(player_info) -> str:
    ...
    champ_icon = champion_service.get_icon(champ_id)
...
def create_my_team_panel(team_data) -> str:
    return "".join([create_my_team_player(player) for player in team_data])

async def create_their_team_player(player_info) -> str:
    champ_id = player_info["championId"]
    champ_icon = champion_service.get_icon(champ_id)
    counter_ids = await champion_service.get_counter_ids(champ_id)
    counters = "".join([f'<img src="{champion_service.get_icon(c_id)}" class="player-summoner">' for c_id in counter_ids])
...
async def create_their_team_panel(team_data) -> str:
    return "".join([await create_their_team_player(player) for player in team_data])

async def update_teams(event: WebsocketEventResponse):
    push(f'<div id="my-team" hx-swap-oob="true">{create_my_team_panel( event.data["myTeam"])}</div>')
    push(f'<div id="their-team" hx-swap-oob="true">{await create_their_team_panel(event.data["theirTeam"])}</div>')
...
def create_team_bans_panel(team_data: list[int]) -> str:
    return "".join([f'<img src="{champion_service.get_icon(ban_id)}" class="bench-champ-icon">' for ban_id in team_data])

def update_bans(event: WebsocketEventResponse):
    my_team_bans, their_team_bans = gather_bans_from_actions(event.data["actions"])
    push(f'<div id="my-team-bans" hx-swap-oob="true">{create_team_bans_panel(my_team_bans)}</div>')
    push(f'<div id="their-team-bans" hx-swap-oob="true">{create_team_bans_panel(their_team_bans)}</div>')
